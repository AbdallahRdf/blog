        ...
        <>
            <nav className='transition-all duration-500 ease-in-out bg-zinc-50 dark:bg-transparent fixed top-2 border border-zinc-200 dark:border-none w-full max-w-[1280px] rounded-xl z-50'>
                ...
                <ul className='rounded-xl px-4 py-1 sm:py-3 flex items-center gap-3 sm:gap-6 relative z-50'>
                    ...
                    {/* Search button */}
                    <li>
                        <button onClick={handleSearchClick} className='transition-all duration-500 ease-in-out hover:scale-110 hover:underline dark:text-zinc-50 py-3 rounded-lg text-sm md:text-lg font-semibold flex items-center gap-2'>
                            <Search className='inline size-4 sm:size-5' /> Search
                        </button>
                    </li>
                    ...
                    >
                        {
                            isDarkMode
                                ?
                                <Sun className='size-6 sm:size-7 text-slate-50' />
                                :
                                <Moon className='size-6 sm:size-7 text-slate-900' />
                        }
                    </button>

                    {
                        (accessToken)
                            ...
                            <>
                                <li>
                                    <button className='transition-all duration-500 ease-in-out dark:hover:scale-125 hover:bg-zinc-200 dark:hover:bg-transparent rounded-full p-2 text-black dark:text-zinc-50'>
                                        <Bell className='hover:fill-black dark:hover:fill-white size-5 sm:size-6' />
                                    </button>
                                </li>
                                <li>
                                    <button ref={UserAvatarBtnRef} onClick={handleClick} className='transition-all duration-500 ease-in-out dark:hover:scale-125 hover:bg-zinc-200 dark:hover:bg-transparent rounded-full p-2 text-black dark:text-zinc-50'>
                                        {
                                            user?.profileImage
                                                ?
                            ...
                            :
                            <li>
                                <Link to="/auth/login" className='transition-all duration-500 ease-in-out bg-white text-slate-900 hover:bg-zinc-800 hover:text-zinc-200 hover:border-zinc-800 dark:hover:border-zinc-50 dark:hover:bg-transparent dark:hover:text-slate-50  py-2 px-5 md:py-4 md:px-6 rounded-lg border-2 text-base font-semibold'>
                                    LogIn
                                </Link>
                            </li>
                    }
